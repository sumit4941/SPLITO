...
This model covers the API, PWA/offline store, Oracle, worker, private storage,
...
API/worker and Oracle; app and private storage; worker and external providers;
...
| Financial tampering/rounding                 | Decimal strings/BigInt, deterministic allocation, server recomputation, balanced journal, immutable revisions, reconciliation                                     | Domain and schema foundations; Oracle transaction tests pending                                                                                                                                                               |
| Duplicate/replayed mutation                  | Actor+operation+key uniqueness, request digest match, outcome written in same transaction                                                                         | `SPLITO_IDEMPOTENCY_KEYS`; integration race tests pending                                                                                                                                                                     |
| Concurrent lost update                       | `If-Match`, row locks in stable order, version predicate, stale simplification rejection                                                                          | Schema versions and API rule; concurrency tests pending                                                                                                                                                                       |
| SQL injection                                | Static SQL templates, bind every value, validated identifier allowlist for owner schema only                                                                      | Migration scripts and owner-schema validation; repository query audit required                                                                                                                                                |
...
| Outbox/job double effect                     | Lease, deduplication key, bounded retries, provider idempotency, dead-letter visibility                                                                           | Durable tables and worker foundation; failure injection pending                                                                                                                                                               |
| Log/error leakage                            | Structured redaction, request IDs, production-safe errors, no SQL binds/provider bodies/PII                                                                       | Pino redaction foundation; automated log-content test pending                                                                                                                                                                 |
| Database privilege abuse                     | Separate owner/runtime users, object grants, PDB service, TCPS, no public synonyms, no runtime DDL                                                                | Bootstrap and grants; production DBA review required                                                                                                                                                                          |
