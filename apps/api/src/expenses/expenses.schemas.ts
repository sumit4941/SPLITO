...
const base = z
  .object({
    ...
    payers: z
      ...
      .min(1)
      .max(1_000),
    originalSplitInputs: z.array(originalParticipant).min(1).max(1_000).optional(),
  })
  .strict();

export const expenseMutationSchema = z.discriminatedUnion('splitMethod', [
  base.extend({
    splitMethod: z.literal('equal'),
    beneficiaries: z.array(participant).min(1).max(1_000),
  }),
  base.extend({
    splitMethod: z.literal('exact'),
    beneficiaries: z.array(exactParticipant).min(1).max(1_000),
  }),
  base.extend({
    splitMethod: z.literal('percentage'),
    beneficiaries: z.array(percentageParticipant).min(1).max(1_000),
  }),
  base.extend({
    splitMethod: z.literal('shares'),
    beneficiaries: z.array(sharesParticipant).min(1).max(1_000),
  }),
  base.extend({
    splitMethod: z.literal('adjustments'),
    beneficiaries: z.array(adjustmentParticipant).min(1).max(1_000),
  }),
]);
